...
func (tx *Tx) init() {
	...
	tx.ImageOfUser = NewImageOfUserClient(tx.config)
	tx.ItemHasOwner = NewItemHasOwnerClient(tx.config)
	tx.Order = NewOrderClient(tx.config)
	tx.ProductQuote = NewProductQuoteClient(tx.config)
	tx.TypeOfGoods = NewTypeOfGoodsClient(tx.config)
...
// applies a query, for example: Goods.QueryXXX(), the query will be executed
