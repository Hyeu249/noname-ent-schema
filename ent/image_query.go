...
func (iq *ImageQuery) QueryImageOfUser() *ImageOfUserQuery {
	query := (&ImageOfUserClient{config: iq.config}).Query()
...
func (iq *ImageQuery) Clone() *ImageQuery {
	...
	return &ImageQuery{
		...
		withImageOfUser: iq.withImageOfUser.Clone(),
...
func (iq *ImageQuery) WithImageOfUser(opts ...func(*ImageOfUserQuery)) *ImageQuery {
	query := (&ImageOfUserClient{config: iq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	iq.withImageOfUser = query
	return iq
}
...
func (iq *ImageQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Image, error) {
	var (
		...
		_spec       = iq.querySpec()
		loadedTypes = [1]bool{
			iq.withImageOfUser != nil,
		}
	)
	if iq.withImageOfUser != nil {
		withFKs = true
	}
	if withFKs {
	...
	if query := iq.withImageOfUser; query != nil {
		if err := iq.loadImageOfUser(ctx, query, nodes, nil,
			func(n *Image, e *ImageOfUser) { n.Edges.ImageOfUser = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (iq *ImageQuery) loadImageOfUser(ctx context.Context, query *ImageOfUserQuery, nodes []*Image, init func(*Image), assign func(*Image, *ImageOfUser)) error {
	ids := make([]uuid.UUID, 0, len(nodes))
	nodeids := make(map[uuid.UUID][]*Image)
	for i := range nodes {
		if nodes[i].image_of_user_image == nil {
			continue
		}
		fk := *nodes[i].image_of_user_image
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "image_of_user_image" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
