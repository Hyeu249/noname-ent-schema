...
func (oq *OrderQuery) QueryItemHasOwner() *ItemHasOwnerQuery {
	query := (&ItemHasOwnerClient{config: oq.config}).Query()
...
func (oq *OrderQuery) Clone() *OrderQuery {
	...
	return &OrderQuery{
		...
		withItemHasOwner: oq.withItemHasOwner.Clone(),
...
func (oq *OrderQuery) WithItemHasOwner(opts ...func(*ItemHasOwnerQuery)) *OrderQuery {
	query := (&ItemHasOwnerClient{config: oq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	oq.withItemHasOwner = query
	return oq
}
...
func (oq *OrderQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Order, error) {
	var (
		...
		_spec       = oq.querySpec()
		loadedTypes = [1]bool{
			oq.withItemHasOwner != nil,
		}
	)
	if oq.withItemHasOwner != nil {
		withFKs = true
	}
	if withFKs {
	...
	if query := oq.withItemHasOwner; query != nil {
		if err := oq.loadItemHasOwner(ctx, query, nodes, nil,
			func(n *Order, e *ItemHasOwner) { n.Edges.ItemHasOwner = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (oq *OrderQuery) loadItemHasOwner(ctx context.Context, query *ItemHasOwnerQuery, nodes []*Order, init func(*Order), assign func(*Order, *ItemHasOwner)) error {
	ids := make([]uuid.UUID, 0, len(nodes))
	nodeids := make(map[uuid.UUID][]*Order)
	for i := range nodes {
		if nodes[i].order_item_has_owner == nil {
			continue
		}
		fk := *nodes[i].order_item_has_owner
	...
	query.Where(itemhasowner.IDIn(ids...))
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "order_item_has_owner" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
